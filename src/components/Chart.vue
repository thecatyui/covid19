...
export default {
    ...
    const arrayNewConfirmed = localData.map((fullNewConfirmed) => fullNewConfirmed.newConfirmed).slice(0,13)
    const arrayNewDeaths = localData.map((fullNewConfirmed) => fullNewConfirmed.newDeaths).slice(0,13)
    const arrayDate = localData.map((fullDate) => fullDate.date).slice(0,13)
